...
public class EOH_drainFluid_Mixin {
    ...
    @Redirect(
        method = "onPreTick",
        at = @At(value = "INVOKE", target = "Lcom/github/technus/tectech/thing/metaTileEntity/multi/GT_MetaTileEntity_EM_EyeOfHarmony;drainFluidFromHatchesAndStoreInternally()V"))
    private void drainFluid(GT_MetaTileEntity_EM_EyeOfHarmony instance){
        boolean shouldDrain = instance.getBaseMetaTileEntity().isAllowedToWork();
        if (shouldDrain){
